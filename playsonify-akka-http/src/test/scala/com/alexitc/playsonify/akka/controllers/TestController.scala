...
class TestController(implicit mat: Materializer) extends CustomJsonController {
  ...
  def authenticatedNoInputRoutes = pathPrefix("authenticated") {
    path("model") {
      get {
  ...
  def authenticatedWithInputRoutes = pathPrefix("authenticated-input") {
    path("model") {
      post {
  ...
  def publicNoInputRoutes = pathPrefix("no-input") {
    path("model") {
      get {
        public { ctx =>
  ...
  def publicWithInputRoutes = pathPrefix("input") {
    path("model") {
      post {
  ...
  def exceptionResponse: FutureApplicationResult[CustomModel] = Future.failed(new RuntimeException("Unknown failure"))
}
