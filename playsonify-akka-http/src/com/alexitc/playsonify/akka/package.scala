...
package object akka {
  ...
  case object MalformedJsonError extends InputValidationError {

    override def toPublicErrorList[L](i18nService: I18nService[L])(implicit lang: L): List[PublicError] = {
      val error = GenericPublicError("Invalid JSON")
      List(error)
    }
  }

  case object RouteNotFoundError extends NotFoundError {

    override def toPublicErrorList[L](i18nService: I18nService[L])(implicit lang: L): List[PublicError] = {
      val error = GenericPublicError("Unknown route")
      List(error)
    }
  }
}
