...
class FieldOrderingParserSpec extends WordSpec with MustMatchers {
  ...
  "from" should {
    ...
    "reject unknown field" in {
      ...
      val expected = Bad(UnknownOrderingFieldError).accumulating
      val result = parser.from(query)
    ...
    "reject unknown ordering condition" in {
      ...
      val expected = Bad(InvalidOrderingConditionError).accumulating
      val result = parser.from(query)
    ...
    "accumulate errors on unknown field and ordering condition" in {
      ...
      val expected = Bad(Every(UnknownOrderingFieldError, InvalidOrderingConditionError))
      val result = parser.from(query)
    ...
    "reject bad ordering format" in {
      ...
      val expected = Bad(InvalidOrderingFormatError).accumulating
      val result = parser.from(query)
