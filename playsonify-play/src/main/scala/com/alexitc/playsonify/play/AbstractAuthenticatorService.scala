...
/** Allow to authenticate a request and map it to a value with type [[A]].
  *
  * A dummy service that gets the user id from the "Authorization" header or uses -1 when the header is not present
  * could look like this, NEVER USE THIS PIECE OF CODE:
  * {{{
